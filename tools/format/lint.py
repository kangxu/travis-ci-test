...
def iter_files():
    for item in git("diff", "--name-status", "HEAD~1").split("\n"):
        status = item.split("\t")
        if status[0].strip() != "D":
            yield item
...
def check_parsed(path, f):
    ...
    if source_file.testharness_nodes:
        ...
        testharnessreport_nodes = source_file.root.findall(".//{http://www.w3.org/1999/xhtml}script[contains(@src, 'testharnessreport.js')")
...
path_lints = [check_path_length, check_filename_space]
